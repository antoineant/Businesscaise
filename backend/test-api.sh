...
echo ""
echo "You can now:"
echo "  - View game in database: psql -U postgres -d businesscaise -c 'SELECT * FROM games;'"
echo "  - Check sessions: psql -U postgres -d businesscaise -c 'SELECT session_number, day, period, status FROM sessions WHERE game_id = '\''$GAME_ID'\'';'"
